...
### 3.7 Ключевые метрики для мониторинга (дашборд)

Рекомендуемый набор графиков для **Prometheus + Grafana** (или просто выводимых в лог-окно приложения):

| Категория | Метрика | Зачем | Целевое значение / Алерт |
|-----------|---------|-------|--------------------------|
| **Производительность** | `p99 latency` обработки файла (от события до перемещения) | Не раздражать пользователя | < 10 сек (если > 30 сек — алерт) |
| | `llm_inference_duration_seconds` (гистограмма) | Мониторинг скорости модели | < 5 сек (p95) |
| | `queue_length` (текущая длина очереди) | Не допустить накопления | > 50 — предупреждение, > 200 — критический алерт |
| **Качество классификации** | `low_confidence_rate` (доля файлов с confidence < 0.6) | Оценить, как часто модель неуверена | < 20% |
| | `fallback_triggered_total{reason="llm_parse_error"}` | Частота галлюцинаций | < 5% от всех файлов |
| | `user_feedback_negative_rate` (доля нажатий «Неправильно») | Реальная точность с точки зрения пользователя | < 10% |
| **Надёжность** | `files_failed_total{reason="move_permission"}` | Проблемы с правами | 0 (любое значение > 0 за час — алерт) |
| | `llm_health_check_failed` (binary) | Доступность LLM | 0 (если 1 в течение 3 проверок — fallback) |
| **Безопасность** | `scripts_high_risk_detected_total` | Сколько раз найден опасный скрипт | Мониторинг тренда (не алерт) |
| | `user_confirmation_denied_total` | Отказы пользователя перемещать скрипты | Логировать, но не алертить |

**Пример дашборда (текстовое описание для разработчика)**:
- Верхняя панель: `files_processed_total` (счётчик), `queue_length` (текущее), `llm_available` (зелёный/красный).
- График 1: **Латентность** — p50, p95, p99 за последний час.
- График 2: **Качество** — доля low_confidence и fallback’ов по часам.
- График 3: **Ошибки** — количество failed по типам (permission, timeout, disk full).
- Таблица внизу: последние 10 файлов с low_confidence (путь, предложенная категория, причина).

**Где смотреть**:
- В PoC метрики пишутся в JSON-логи и могут быть визуализированы через простой скрипт (например, `python -m http.server` и страница с графиками на Plotly).
- Для production — экспорт в Prometheus через `prometheus_client` и Grafana.
