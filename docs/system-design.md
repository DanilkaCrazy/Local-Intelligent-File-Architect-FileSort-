...
### 3.1 Retriever (не применим, но есть кэш)

- **Источники кэша:** хеш файла → категория/папка.
- **Индекс:** SQLite таблица `cache(hash TEXT PRIMARY KEY, category TEXT, folder TEXT, confidence REAL, created TIMESTAMP)`.
- **Поиск:** при получении файла вычисляем SHA-256, если есть в кэше и не старше 30 дней – используем без вызова LLM.
- **Ограничения:** только для файлов, которые уже были обработаны; не используем для скриптов с высоким риском (всегда свежий анализ).
